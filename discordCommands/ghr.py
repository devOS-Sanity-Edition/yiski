...
class FunDiscord(
    ...
    async def ghr(self, ctx, name: str, repo: str):  # self always needs to be before ctx in cogs
        ...
        if request.status_code == 200:
            ...
            if githubToken != None:
                embed.add_field(name="Is a GitHub Token Being Used?", value="Yes...\nPlease make sure that you have a valid token, or contact the bot owner to make sure that they have a valid token.\nOr that you somehow managed exceeded the rate limit with one of those?????") 
            else:
                embed.add_field(name="Is a GitHub Token Being Used?", value="No...\nUse a GitHub Token in your config.json5, or contact the bot owner to get one put into config.json5!\n You will not get ratelimited as often!")
        else:
